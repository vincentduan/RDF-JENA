...
private function xmlCompleteHandler(event:Event):void {
	if (event is ResultEvent) {
		...
		Alert.show((event as FaultEvent).fault.toString(), "没有找到配置文件Config");
	}
...
private function exampleCompleteHandler(event:Event):void {
	if (event is ResultEvent) {
		...
		Alert.show((event as FaultEvent).fault.toString(), "没有找到范例文件");
	}
...
private function statusChangedHandler(event:Event):void {
	statusLabel.text = "状态: " + StatusModel.getInstance().message;
...
private function urlClickHandler(event:MouseEvent):void{
	...
	Alert.show(url, "下面的URL已经保存到你的剪贴板");
}
...
public function loadExample2(example:Example):void {
	...
	if (ConnectionModel.getInstance().sparqlConfig != example.endpointConfig) {
		if (example.endpointConfig != null) {
			Alert.show("你选择的端点已被设置成\"" + example.endpointConfig.name + "\"。\n你可以在设置菜单中将端点改回为\"" + ConnectionModel.getInstance().sparqlConfig.name + "\"。", "端点被改变", Alert.OK + Alert.NONMODAL);
			ConnectionModel.getInstance().sparqlConfig = example.endpointConfig;
		}else {
			searchPossible = false;
...
public function findRelations():void {
	...
	if (graphModel.givenNodes.isEmpty()) {
		...
		Alert.show("在查询新的关系前，您是否要清除旧的查询结果？", "清除", Alert.YES + Alert.NO, this, dispatchCloseEvent);
	}
}
...
private function findRelationsImmediately():void {
	...
	if (isInputValid()) {
		...
		if (isInputUnique()) {
			...
			myConnection.findRelations(between, 15, ConnectionModel.getInstance().sparqlConfig.maxRelationLength + 1, resultParser);
